...
function drawHUD() {
  ...
  function meterBar(x, meter, maxMeter, flip) {
    ...
    // special-ready threshold marker -- a tick at the meter fraction needed to throw
    // a special, so it's clear at a glance how close you are to having one available
    const readyPct = SPECIAL_METER_COST / maxMeter;
    const readyX = flip ? x + w * (1 - readyPct) : x + w * readyPct;
    ctx.strokeStyle = meter >= SPECIAL_METER_COST ? '#ffe066' : '#999';
    ctx.lineWidth = 2;
    ctx.beginPath(); ctx.moveTo(readyX, 45); ctx.lineTo(readyX, 55); ctx.stroke();
    ctx.lineWidth = 1;
